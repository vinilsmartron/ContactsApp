...
public class PeopleActivity extends ContactsActivity implements
	...
	private void createViewsAndFragments(Bundle savedState) {
		...
		mTabTitles[TabState.GROUPS] = "Groups";
		...
		// fragments
		// existing.
		mFavoritesFragment = (ContactTileListFragment) fragmentManager
	...
	private class ContactsUnavailableFragmentListener implements
		...
		public void onImportContactsFromFileAction() {
			ImportExportDialogFragment.show(getFragmentManager(),
					areContactsAvailable(), PeopleActivity.class);
		}
	...
	private class GroupDetailFragmentListener implements
			GroupDetailFragment.Listener {
		...
		public void onGroupSizeUpdated(String size) {
			// Nothing needs to be done here because the size will be displayed
			// in the detail
			// fragment
		}
		...
		public void onGroupTitleUpdated(String title) {
			// Nothing needs to be done here because the title will be displayed
			// in the detail
			// fragment
		}
		...
		public void onAccountTypeUpdated(String accountTypeString,
				String dataSet) {
			// Nothing needs to be done here because the group source will be
			// displayed in the
			// detail fragment
		}
		...
		public void onContactSelected(Uri contactUri) {
			// Nothing needs to be done here because either quickcontact will be
			// displayed
	...
	public boolean isOptionsMenuChanged() {
		...
		if (mGroupDetailFragment != null
				&& mGroupDetailFragment.isOptionsMenuChanged()) {
			return true;
		}
	...
	public boolean onPrepareOptionsMenu(Menu menu) {
		...
		if (isSearchMode) {
			...
			case TabState.GROUPS:
				// Do not display the "new group" button if no accounts are
				// available
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.menu_delete: {
			...
			intent.putExtra(
					AccountFilterActivity.KEY_EXTRA_CONTACT_LIST_FILTER, filter);
			...
			if (resultCode == RESULT_OK) {
				...
				if (!mContactList.isEmpty()) {
					if (!ImportExportDialogFragment.isExportingToSIM()) {
						ImportExportDialogFragment.destroyExportToSimThread();
						ExportToSimThread exportThread = new ImportExportDialogFragment()
								.createExportToSimThread(
										ImportExportDialogFragment.mExportSub,
										mContactList, PeopleActivity.this);
						exportThread.start();
					}
				}
			}
